...
function datesFor(range: LogDateRange) { const now = new Date(); const start = new Date(now); start.setHours(0, 0, 0, 0); if (range === 'week') start.setDate(start.getDate() - (start.getDay() === 0 ? 6 : start.getDay() - 1)); if (range === 'month') start.setDate(1); return range === 'all' ? {} : { from: start.toISOString(), to: range === 'today' ? new Date(start.getTime() + 86_400_000).toISOString() : undefined }; }
...
export default async function LogsPage({ searchParams }: PageProps) {
  ...
  const db = createDatabase(); const members = db.listUsers(); const result = db.queryWorkLogs({ authorId: member === 'all' ? undefined : member, query, cursor, limit: 50, ...datesFor(selectedRange) }); db.close();
